...
// DEBUG: generate-export - Full rewrite (JWT auth + JSON {html}) - 2026-02-16
...
function parseDateRange(startDate?: string, endDate?: string): DateRange | null {
  ...
  // Full-day inclusive, in UTC to avoid timezone surprises
...
function jsonError(status: number, error: string, details?: string) {
  return json(status, { error, ...(details ? { details } : {}) });
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseAnonKey || !supabaseServiceKey) {
      return jsonError(500, "Server configuratie fout", "Missing SUPABASE_URL / SUPABASE_ANON_KEY / SUPABASE_SERVICE_ROLE_KEY");
    }
    ...
    if (authError || !authData?.user) {
      ...
      return jsonError(401, "Authenticatie mislukt", authError?.message || "Geen gebruiker gevonden");
    }
    ...
    if (exportType === "child" && !childId) return jsonError(400, "childId is verplicht bij exportType=child");

    const range: DateRange | null = exportType === "date_range" ? parseDateRange(startDate, endDate) : null;
    if (exportType === "date_range" && !range) return jsonError(400, "Ongeldig datumbereik (startDate/endDate)");

    // ✅ Service client (bypass RLS), only AFTER auth succeeded
    ...
    console.log("Fetching data with filters:", {
      exportType,
      childId,
      range: range ? `${range.startYmd} to ${range.endYmd}` : null,
    });

    // CHILDREN
    let childrenQ = serviceSupabase.from("children").select("*").eq("family_id", familyId);
    if (exportType === "child" && childId) childrenQ = childrenQ.eq("id", childId);
    childrenQ = childrenQ.order("first_name");
    const { data: childrenData, error: childrenErr } = await childrenQ;
    if (childrenErr) {
      console.error("Children fetch error:", childrenErr);
      return jsonError(500, "Fout bij ophalen van kinderen", childrenErr.message);
    }
    const childrenToExport = childrenData || [];
    ...
    if (exportType === "child" && childId) eventsQ = eventsQ.eq("child_id", childId);

    if (exportType === "date_range" && range) {
      // overlap: event starts before end AND (no end OR end after start)
      eventsQ = eventsQ.lte("start_at", range.endIso).or(`end_at.is.null,end_at.gte.${range.startIso}`);
    }
    ...
    if (eventsErr) {
      ...
      return jsonError(500, "Fout bij ophalen van agenda", eventsErr.message);
    }
    ...
    if (exportType === "child" && childId) logsQ = logsQ.eq("child_id", childId);
    ...
    const { data: logEntries, error: logsErr } = await logsQ;
    if (logsErr) {
      console.error("Log entries fetch error:", logsErr);
      return jsonError(500, "Fout bij ophalen van logboek", logsErr.message);
    }

    const logEntriesWithUsers = await Promise.all(
      (logEntries || []).map(async (log: any) => {
        const { data: u } = await serviceSupabase.from("users").select("name").eq("id", log.created_by).maybeSingle();
    ...
    if (exportType === "child" && childId) requestsQ = requestsQ.eq("child_id", childId);
    ...
    const { data: requests, error: reqErr } = await requestsQ;
    if (reqErr) {
      console.error("Requests fetch error:", reqErr);
      return jsonError(500, "Fout bij ophalen van verzoeken", reqErr.message);
    }

    const requestsWithUsers = await Promise.all(
      (requests || []).map(async (r: any) => {
        const { data: u } = await serviceSupabase.from("users").select("name").eq("id", r.created_by).maybeSingle();
        return { ...r, user_name: u?.name || "Onbekend" };
      })
    );

    // QUESTIONS (met fallback als child_id niet bestaat)
    let questions: any[] = [];
    {
      ...
      if (exportType === "child" && childId) q1 = q1.eq("child_id", childId);
      if (exportType === "date_range" && range) q1 = q1.gte("created_at", range.startIso).lte("created_at", range.endIso);
      ...
      if (!res1.error) {
        ...
        if (msg.includes("child_id")) {
          ...
          if (exportType === "date_range" && range) q2 = q2.gte("created_at", range.startIso).lte("created_at", range.endIso);

          q2 = q2.order("created_at", { ascending: false });

          const res2 = await q2;
          if (res2.error) {
            console.error("Questions fetch error:", res2.error);
            return jsonError(500, "Fout bij ophalen van vragen", res2.error.message);
          }
          questions = res2.data || [];
        } else {
          console.error("Questions fetch error:", res1.error);
          return jsonError(500, "Fout bij ophalen van vragen", res1.error.message);
        }
      }
    }
    ...
    const questionsWithDetails = await Promise.all(
      (questions || []).map(async (question: any) => {
        const { data: helper } = await serviceSupabase.from("users").select("name").eq("id", question.helper_id).maybeSingle();
        ...
        const { data: answers, error: ansErr } = await answersQ;
        if (ansErr) {
        ...
        const answersWithUsers = await Promise.all(
          (answers || []).map(async (answer: any) => {
            const { data: parent } = await serviceSupabase.from("users").select("name").eq("id", answer.parent_id).maybeSingle();
    ...
    // AUDIT LOGS
    let auditQ = serviceSupabase.from("audit_logs").select("*").eq("family_id", familyId);
    if (exportType === "date_range" && range) auditQ = auditQ.gte("created_at", range.startIso).lte("created_at", range.endIso);
    auditQ = auditQ.order("created_at", { ascending: false }).limit(100);

    const { data: auditLogs, error: auditErr } = await auditQ;
    if (auditErr) {
      console.error("Audit logs fetch error:", auditErr);
      return jsonError(500, "Fout bij ophalen van audit logs", auditErr.message);
    }
    ...
    // ✅ Return JSON, werkt het best met supabase.functions.invoke()
...
function generateHTML(data: any): string {
  ...
  const exportTypeLabel =
    exportType === "full" ? "Volledig dossier" : exportType === "child" ? "Per kind" : "Datumbereik";
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .card-meta { font-size: 13px; color: #64748b; margin-bottom: 12px; }

    .card-content { font-size: 14px; color: #475569; line-height: 1.6; }
  ...
  <div class="container">
    <div class="header">
      ...
      <div class="meta">
        ...
        <strong>Type:</strong> ${exportTypeLabel}
      </div>
    </div>
        ...
        ${events
          ...
          <div class="card">
            ...
            <div class="card-meta">
              ...
              ${
                event.children
                  ? `<span class="child-tag" style="background: ${event.children.color}22; color: ${event.children.color};">${event.children.first_name}</span>`
                  : ""
              }
              <br>
        ...
        ${logEntries
          ...
          <div class="card">
            ...
            <div class="card-meta">
              ...
              ${
                log.children
                  ? `<span class="child-tag" style="background: ${log.children.color}22; color: ${log.children.color};">${log.children.first_name}</span>`
                  : ""
              }
              <br>
        ...
        ${requests
          ...
          <div class="card">
            ...
            <div class="card-meta">
              ...
              ${
                request.children
                  ? `<span class="child-tag" style="background: ${request.children.color}22; color: ${request.children.color};">${request.children.first_name}</span>`
                  : ""
              }
              <br>
      ...
      <div class="section page-break">
        ...
        ${questions
          ...
          <div class="card">
            ...
            <div class="card-meta">
              <span class="badge ${
                question.status === "ANSWERED" ? "badge-green" : question.status === "OPEN" ? "badge-yellow" : "badge-gray"
              }">
          ...
          <tbody>
            ${auditLogs
              ...
              <tr>
                ...
                <td style="font-family: monospace; font-size: 11px;">${String(log.entity_id || "").substring(0, 8)}...</td>
              </tr>
            `
              )
              .join("")}
