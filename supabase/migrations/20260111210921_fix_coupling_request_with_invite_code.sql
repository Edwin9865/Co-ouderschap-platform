/*
  # Fix coupling request with invite code
  
  ## Problem
  When sending a coupling request, the client cannot access family_members
  of other families due to RLS policies. This prevents the coupling process.
  
  ## Solution
  Create a SECURITY DEFINER function that handles the entire coupling request
  process server-side, bypassing RLS where needed.
  
  ## Changes
  1. Create send_coupling_request function
  2. This function validates the invite code and creates the coupling request
*/
